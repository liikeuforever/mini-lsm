...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        for table in snapshot.l0_sstables.iter() {
            ...
            if key_within(
                key,
                table.first_key().for_testing_key_ref(),
                table.last_key().for_testing_key_ref(),
            ) {
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let old_memtables;
        {
            ...
            old_memtables = std::mem::replace(&mut snapshot.memtable, memtable);
            snapshot.imm_memtables.insert(0, old_memtables.clone());
    ...
    pub fn scan(
        ...
        for table_id in snapshot.l0_sstables.iter() {
            ...
            if range_overlap(
                lower,
                upper,
                table.first_key().as_key_slice().into_inner(),
                table.last_key().as_key_slice().into_inner(),
            ) {
